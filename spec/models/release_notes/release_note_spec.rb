  ...
  RSpec.describe ReleaseNote, type: :model do
    ...
    describe "is valid when" do
      ...
      it "it has no release_note_items" do
        @release_note = FactoryBot.create(:release_note, user_id: Faker::Number.number(2))
      ...
      it "it has one release_note_item" do
        ...
        1.times do
          FactoryBot.create(:release_note_item, change_type_id: @change_type.id, user_id: Faker::Number.number(2), release_note_id: @release_note.id)
        end
    ...
    describe "returns the correct records when using a scope" do
      ...
      before do
        ...
        @release_note_1 = FactoryBot.create(:release_note, user_id: Faker::Number.number(2), published: true)
        @release_note_2 = FactoryBot.create(:release_note, user_id: Faker::Number.number(2), published: true)
      ...
      context "when there are multiple release notes and not all are published" do
        it "only returns published release_notes through the published scope" do
          ...
          rand(2..5).times do
            FactoryBot.create(:release_note_item, change_type_id: change_type.id, user_id: Faker::Number.number(2), release_note_id: release_note_1.id)
          end

          release_note_2 = FactoryBot.create(:release_note, user_id: Faker::Number.number(2), published: false)

          rand(2..5).times do
            FactoryBot.create(:release_note_item, change_type_id: change_type.id, user_id: Faker::Number.number(2), release_note_id: release_note_2.id)
          end
